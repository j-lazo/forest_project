...
def main(_argv):
    ...
    train_ds = dl.tf_dataset_asl_scanns(dict_train, radius=selection_radius, num_channels=13, training_mode=True, data_type=data_type, augment=True)
    val_ds = dl.tf_dataset_asl_scanns(dict_val, radius=selection_radius, num_channels=13, training_mode=True, data_type=data_type, augment=True)
    new_test_ds = dl.tf_dataset_asl_scanns(dict_test, radius=selection_radius, num_channels=13, training_mode=False, batch_size=1, data_type=data_type, augment=False,
    ...
    if name_model == 'Simple_CNN':
        model = cm.D_model(5, input_shape=(selection_radius*2+1,selection_radius*2+1,13))
        print('Compiling simple model')
        model.compile(optimizer=opt, loss=loss_fn, metrics=metrics)

    elif name_model == 'Multi-sized_input_CNN':
        model = cm.multi_input_model(5, input_shape=(selection_radius*2+1,selection_radius*2+1,13))
        print('Compiling Multi-sized input model')
        model.compile(optimizer=opt, loss=loss_fn, metrics=metrics)
        model.summary()

    elif name_model == 'Simple_Conv':
        model = cm.simple_conv_model(5, input_shape=(selection_radius*2+1,selection_radius*2+1,13))
